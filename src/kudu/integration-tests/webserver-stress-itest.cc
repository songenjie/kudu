...
TEST_F(KuduTest, TestWebUIDoesNotCrashCluster) {
...
#endif
  opts.num_masters = 3;
  opts.extra_master_flags.emplace_back("-metrics_merge_inject_duration_ms=50");
  opts.extra_tserver_flags.emplace_back("-metrics_merge_inject_duration_ms=50");
  ...
  // Wait util all checkers finished.
...
// Tests that fetch metrics while deleting tablet doesn't cause any deadlock.
TEST_F(KuduTest, TestFetchMetricsWhileDeletingTableNotCauseDeadlock) {
  ExternalMiniClusterOptions opts;
#ifdef __linux__
  // We can only do explicit webserver ports on Linux, where we use
  // IPs like 127.x.y.z to bind the minicluster servers to different
...
#endif
  opts.num_masters = 3;
  opts.extra_tserver_flags.emplace_back("-metrics_merge_inject_duration_ms=1000");
