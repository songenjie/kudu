...
if [[ ! -f ${bin_path}kudu-master ]]; then
  echo "ERROR: kudu-master not found in ${bin_path}"
  exit 1
fi

if [[ ! -f ${bin_path}kudu-tserver ]]; then
  echo "ERROR: kudu-tserver not found in ${bin_path}"
  exit 1
fi
...
do
  ...
  nohup ${bin_path}kudu-master --fs_wal_dir=${deploy_path}m${i}/wal --log_dir=${deploy_path}m${i}/log --rpc_bind_addresses=${local_ip}:$[${base_m_rpc_port}+$i] --webserver_port=$[${base_m_webserver_port}+$i] --master_addresses=${local_ip}:$[${base_m_rpc_port}+1],${local_ip}:$[${base_m_rpc_port}+2],${local_ip}:$[${base_m_rpc_port}+3] > ${deploy_path}m${i}/mlog 2>&1 &
done
...
do
  ...
  nohup ${bin_path}kudu-tserver --fs_data_dirs=${deploy_path}t${i}/data --fs_wal_dir=${deploy_path}t${i}/wal --log_dir=${deploy_path}t${i}/log --rpc_bind_addresses=${local_ip}:$[${base_t_rpc_port}+$i] --webserver_port=$[${base_t_webserver_port}+$i] --tserver_master_addrs=${local_ip}:$[${base_m_rpc_port}+1],${local_ip}:$[${base_m_rpc_port}+2],${local_ip}:$[${base_m_rpc_port}+3] > ${deploy_path}t${i}/tlog 2>&1 &
done
