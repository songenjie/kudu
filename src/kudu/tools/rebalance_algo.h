...
class RebalancingAlgo {
 public:
  ...
  // The top-level method of the empty blacklist tserver algorithm. Using
  // information on the current state of the cluster in 'cluster_info', the
  // algorithm gets particular replica movements instructions and populates
  // the output parameter 'moves' that aim to move all replicas on blacklist
  // tservers to other servers.
  //
  // Once this method returns Status::OK(), there would be no replica on blacklist
  // tservers, so these servers would be removed from 'cluster_info'.
  //
  // 'moves' must be non-NULL.
